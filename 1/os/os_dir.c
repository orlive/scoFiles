...
int sort_dateien(t_ele *e1,t_ele *e2)
{
	...
	case 0:					/* dateiname */
		...
		break;
	case 1:				 	/* Größe (kleinste zuerst) */
		i = (int)(e1->size - e2->size);
		break;
	case 2:					/* Größe (größte   zuerst) */
		i = (int)(e2->size - e1->size);
		break;
	case 3:					/* Datum (jüngste zuerst) */
		if(e1->dir=='d' && e2->dir=='d')
			i = 0;
		else
			i = (int)(e2->mod_time - e1->mod_time);
		break;
	case 4:					/* Datum (älteste zuerst) */
		if(e1->dir=='d' && e2->dir=='d')
			i = 0;
		else
...
void hole_dateien(t_dir *d,char *pfad)
{
	...
	qsort(&d->e[0],d->anz,sizeof(d->e[0]),sort_dateien);
}

void hole_dateien2( t_dir *d ) /* erhählt aenderungnen */
{
	int	u;
	t_dir	d2;
	char	name[100];
